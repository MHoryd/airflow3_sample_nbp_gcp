...
from airflow_kata.transformations.nbp_bronze_transformer import get_target_string
from airflow_kata.transformations.nbp_silver_transformer import (
    get_searched_date_strings,
    make_nbp_silver_transformation,
)
from airflow_kata.transformations.nbp_xml_parser import parse_nbp_xml_to_records
...
class NbpCurrencyRates:
    ...
    def get_rates_and_store_in_bronze(
        ...
        target = get_target_string(request_for_rates_list, run_date)
        if target:
            ...
            blob_path = f"{metadata['bronze_path']}year={now.year}"
            f"/month={now.month:02d}/day={now.day:02d}/table.xml"
    ...
    def transform_rates_to_silver_layer(
        ...
        **kwargs: str,
        ...
        searched_year, searched_month, searched_day = get_searched_date_strings(
            logical_date
        )
        bucket_client = GoogleCloudStorageHandler()
        blob_name = f"{metadata['bronze_path']}year=20{searched_year}"
        f"/month={searched_month}/day={searched_day}/table.xml"
        ...
        column_mapping = metadata["column_rename_pattern"]
        transformed_df = make_nbp_silver_transformation(rates_list, column_mapping)
        blob_name = f"{metadata['silver_path']}year=20{searched_year}"
        f"/month={searched_month}/day={searched_day}/transformed.parquet"
